...
export default function MenuSimple({lead, selectedLead}) {
    ...
    useEffect(() => {
        ...
        const fetchCounselors = async () => {
            try {
                const response = await fetch("http://localhost:8080/api/users/counselor");
                if (response.ok) {
    ...
    return (
        <Dropdown>
            ...
            <Menu slots={{ listbox: Listbox }}>
                {counselors.map((item) => <MenuItem onClick={createHandleMenuClick(item._id)}>{item.name}</MenuItem>)}
            </Menu>
        </Dropdown>
    );
}
